...
public class ArizenToolManager {
    ...
    public ArizenToolManager(Context context) {
        ...
        // Routines ecosystem
        tools.put("RoutineTool",      new RoutineTool(context));
    }

    /** Execute all [Tool:param] calls found in aiResponse, return cleaned text */
    ...
    public String getToolDescriptions() {
        ...
        for (ArizenTool t : tools.values()) sb.append(t.describe()).append("; ");
        return sb.toString();
    }

    @SuppressWarnings("unchecked")
    public <T extends ArizenTool> T getTool(Class<T> cls) {
        for (ArizenTool t : tools.values()) if (cls.isInstance(t)) return (T) t;
        return null;
    }
}
