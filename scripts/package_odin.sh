#!/usr/bin/env bash
# ArizenOS Lite — package_odin.sh (FIXED v3)
# Creates Odin-compatible AP.tar.md5 for SM-T295
set -euo pipefail
...
[[ ! -f "$AP_EXTRACTED/system.img" ]] && \
    fail "system.img not found in ap_extracted. Run repack_ap.sh first."

mkdir -p "$OUTPUT_DIR"
log "Packaging: $OUTPUT_NAME"

# ── 1. Collect all partition files ────────────────────────────────────────────
# Odin AP partition can contain: system, boot, recovery, vbmeta, dtbo, userdata, vendor
...
# IMPORTANT: use -C so paths inside tar are bare filenames (no directory prefix)
...
# Samsung format: raw tar bytes + ASCII MD5 (32 bytes, NO newline)
...
# Verify MD5 strip works (self-check)
...
if [[ "$VERIFY_MD5" == "$STORED_MD5" ]]; then
    ok "MD5 self-check PASSED ✓"
else
    warn "MD5 self-check mismatch — this may indicate a packaging issue"
...
echo ""
log "Flash instructions (macOS — Heimdall):"
